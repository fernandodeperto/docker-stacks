#!/usr/bin/env bash

set -Eeuo pipefail
trap cleanup SIGINT SIGTERM ERR EXIT

usage() {
  cat <<EOF
Usage: $(basename "${BASH_SOURCE[0]}") [-h] [-v] -d|--stack-dir stack_directory -s|--stack stack up|down|pull

Docker script to manage Docker Compose stacks.

Available options:

-h, --help      Print this help and exit
-v, --verbose   Print script debug info
-d, --stack_dir Stack directory
-s, --stack     Stack

Available commands:

up
down
pull
EOF
  exit
}

cleanup() {
  trap - SIGINT SIGTERM ERR EXIT
  # script cleanup here
}

setup_colors() {
  if [[ -t 2 ]] && [[ -z "${NO_COLOR-}" ]] && [[ "${TERM-}" != "dumb" ]]; then
    NOFORMAT='\033[0m' RED='\033[0;31m' GREEN='\033[0;32m' ORANGE='\033[0;33m' BLUE='\033[0;34m' PURPLE='\033[0;35m' CYAN='\033[0;36m' YELLOW='\033[1;33m'
  else
    NOFORMAT='' RED='' GREEN='' ORANGE='' BLUE='' PURPLE='' CYAN='' YELLOW=''
  fi
}

msg() {
  echo >&2 -e "${1-}"
}

die() {
  local msg=$1
  local code=${2-1} # default exit status 1
  msg "$msg"
  exit "$code"
}

parse_params() {
  # default values of variables set from params
  dir=""
  stack="*.yml"
  
  while :; do
    case "${1-}" in
    -h | --help) usage ;;
    -v | --verbose) set -x ;;
    --no-color) NO_COLOR=1 ;;
    -d | --stack_dir)
      dir="${2-}"
      shift
      ;;
    -s | --stack)
      stack="${2-}"
      shift
      ;;
    -?*) die "Unknown option: $1" ;;
    *) break ;;
    esac
    shift
  done

  args=("$@")

  # check required params and arguments
  [[ -z "${dir-}" ]] && die "Missing required parameter: dir"
  [[ ${#args[@]} -eq 0 ]] && die "Missing script argument"

  case "${args[0]}" in
    up)
      ls="ls"
      command="up -d -y --remove-orphans"
      ;;
    down)
      ls="ls -r"
      command="down --remove-orphans"
      ;;
    pull)
      ls="ls"
      command="pull"
  esac

  return 0
}

show_params() {
  msg "${GREEN}Read parameters:${NOFORMAT}"
  msg "- dir: ${dir}"
  msg "- stack: ${stack}"
  msg "- argument: ${args[0]-}"
  msg ""
}

parse_params "$@"
setup_colors
show_params

for compose in $($ls ${dir}/${stack}); do
  project="${PWD##*/}-$(basename "$compose" | sed -E 's/^[0-9]+-(.*)-compose\.yml/\1/')"
  docker compose -p "$project" -f "$compose" $command
done

